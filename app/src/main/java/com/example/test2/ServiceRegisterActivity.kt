...
class ServiceRegisterActivity : AppCompatActivity() {
    ...
    private fun registerUser(txtMail: String, txtPassword: String, txtName: String, txtPhone: String, txtConfirm: String, txtService: String) {
        ...
        auth.createUserWithEmailAndPassword(txtMail, txtPassword)
            .addOnCompleteListener(this) { task ->
                if (task.isSuccessful) {
                    ...
                    val userId = user?.uid ?: ""
                    val userInfo = Information(txtMail, txtName, txtPhone, txtService)

                    db.collection("Services").document(userId).set(userInfo).addOnCompleteListener(this) { task ->
                        if(task.isSuccessful) {
